...
/**
 * Allows TinyVG to be implemented in Scene2D.UI Stages via an {@link Image}
 * actor. The Image is backed by a {@link
 * TinyVGRegionDrawable} that can be automatically redrawn when
 * {@link Widget#layout()} is called. The {@link TinyVG} is
 * rendered to a {@link TextureRegion} via a
 * {@link com.badlogic.gdx.graphics.glutils.FrameBuffer} before it is rendered
 * to the screen. autoRedrawing is an intensive operation and should be disabled
 * for large drawables or when many
 * TinyVGRegionImages are used in tandem. The scale of the original TinyVG is
 * respected and automatically sets the
...
public class TinyVGRegionImage extends Image {
    ...
    public void layout() {
        super.layout();
        if (isAutoRedrawing() && drawable != null)
            drawable.update(MathUtils.ceil(getImageWidth()), MathUtils.ceil(getImageHeight()));
    }

    /**
     * Sets a new drawable for the image. The image's pref size is the drawable's
     * min size. If using the image actor's
     * size rather than the pref size, {@link #pack()} can be used to size the image
     * to its pref size.
     *
